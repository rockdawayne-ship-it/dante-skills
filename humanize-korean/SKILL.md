...
# Humanize Korean — Fast Path

한글 텍스트의 "AI 티"를 단일 호출 안에서 탐지·윤문·자체검증한다. 이 스킬은 서브에이전트·외부 스크립트 없이 SKILL.md + `references/` 룰북만으로 동작하는 독립형(fast path) 에디션이다. 정밀 3콜 파이프라인(진단→겨냥 윤문→finalize)이 필요하면 원본 플러그인([epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai))을 설치한다.
...
8. **응답**: 사용자에게 짧게 4가지 반환 — ① 한 줄 상태(`완료. 변경률 X% / 등급 Y / 자체검증 N/6 통과`) ② 핵심 카테고리 탐지 4~6건(before → after) ③ 변경 하이라이트 1건 ④ 등급 B 이하면 "원본 플러그인의 정밀 모드(3콜) 권장" 안내. 윤문본 본문은 응답에 인라인하지 말고 `final.md`에만 저장.

## 등급

- **A**: S1 0건, S2 2건 이하, 변경률 10~25%, 자체검증 6/6.
- **B**: S1 0건, S2 4건 이하, 자체검증 5/6 이상.
- **C/D**: S1 잔존 또는 과윤문 시그널 — 원본 플러그인 정밀 모드 권고.
...
[epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai) (MIT License, Copyright (c) 2026 epoko77-ai)의 fast-path 에디션을 skills.sh 배포용으로 재구성한 파생물이다. fork: [dandacompany/im-not-ai](https://github.com/dandacompany/im-not-ai).
